...
function extractInfoBlocks(
  skill: { resources: Prisma.JsonValue | null } | null | undefined,
): InfoBlock[] {
  if (
    !skill?.resources ||
    typeof skill.resources !== "object" ||
...
router.get(
  ...
  async (
    req: Request<{ roadmapId: string; nodeId: string }>,
    res: Response,
  ) => {
    try {
